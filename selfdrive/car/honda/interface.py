...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[], disable_radar=False):  # pylint: disable=dangerous-default-value
    ...
    if candidate == CAR.CIVIC:
      ...
      ret.longitudinalTuning.kpBP = [0., 5., 10, 20., 35.]
      ret.longitudinalTuning.kpV = [1.6, 1.4, 1.3, 0.9, 0.8]
      ret.longitudinalTuning.kiBP = [0., 5., 12., 20., 27.]
      ret.longitudinalTuning.kiV = [.8, .7, .6, .4, .3]
  ...
  def init(CP, logcan, sendcan):
    if (CP.carFingerprint in (HONDA_BOSCH - HONDA_BOSCH_RADARLESS) and CP.openpilotLongitudinalControl) or self.CP.carFingerprint == CAR.ACURA_RDX_3G:
