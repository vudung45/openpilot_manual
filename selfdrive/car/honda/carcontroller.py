...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    if not self.CP.openpilotLongitudinalControl:
      if self.frame % 2 == 0 and self.CP.carFingerprint not in HONDA_BOSCH_RADARLESS and not radar_disabled:  # radarless cars don't have supplemental message
        idx = self.frame // 2
      ...
      if self.frame % 2 == 0:
        ...
        if self.CP.carFingerprint in HONDA_BOSCH:
          ...
          if not CS.out.cruiseState.enabled and not (CS.CP.pcmCruise and CC.longActive):
            apply_brake = 0.
          pump_on, self.last_pump_ts = brake_pump_hysteresis(apply_brake, self.apply_brake_last, self.last_pump_ts, ts)

          pcm_override = True
          can_sends.append(hondacan.create_brake_command(self.packer, apply_brake, pump_on, pcm_override, pcm_cancel_cmd, fcw_display, idx, self.CP.carFingerprint, CS.stock_brake))
